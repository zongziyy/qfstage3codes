...
import { asyncAddUser, asyncInitUserList, asyncDeleteUser } from "../action/userAction"
...
export default () => {
    ...
    const [myBtn, setMyBtn] = useState({color:"red"});
    ...
    useEffect(async () => {//页面加载完成以后的判断
        if (!userList.length) {
            dispatch(asyncInitUserList());//把数据放到全局
        }
    ...
    let add = () => {
        ...
        dispatch(asyncAddUser({ username, userpwd }));

    }
    let delUser = (id) => {
        dispatch(asyncDeleteUser(id,()=>{
            setMyBtn({color:"green"})
        }));
